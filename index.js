...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    const randomCharacter = characters.charAt(ramdomIndex)

    pasworld += randomCharacter
  }
...
function calculateCircleArea(radius) {
  ...
  let circRadiu = Math.PI * (radius ^ 2)
...
function roundObjectValues(obj) {
  ...
  const roundEntries = arr.map(([key, value]) => {
  ...
  let obj1 = Object.fromEntries(roundEntries)
...
function calculateVolumeCylinder(radius, height) {
  ...
  let cilinValum = Math.PI * Math.pow(radius, 2) * height

  // Округляємо об'єму до найменшого цілого числа числа що більше.
  let valum = Math.ceil(cilinValum)
...
function sumPositiveNumbers(numbers) {
  ...
  for (const num of numbers) {
    ...
    if (Math.sign(num) === 1) {
      posiNumbers.push(num)
    }
  }
  let result = posiNumbers.reduce(
...
function compareAndRound(num1, num2) {
  ...
  let roundUpNum = Math.ceil(maxNum)
...
function estimateInvestment(
  ...
  let roundNum = Math.round(investAmount)
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  let converTotalPrice = Math.fround(totalPrice)
  let converMaxPrice = Math.fround(maxPrice)

  // Порівнюємо, чи не перевищує totalPrice maxPrice.
  let diferentPrice = converTotalPrice === converMaxPrice

  return diferentPrice
}
